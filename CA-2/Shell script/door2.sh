...
kubectl scale deploy door2-deployment --replicas=0
echo "Set relicas=1 to create the pod"
kubectl scale deploy door2-deployment --replicas=1
echo "Sleep for 10 sec to run the container"
sleep 10
newPod=$(kubectl get pods | grep "door2-deployment" | awk '{print $1}')
