...
kubectl scale deploy seccon-deployment --replicas=0
echo "Set relicas=1 to create the pod"
kubectl scale deploy seccon-deployment --replicas=1
echo "Sleep for 10 sec to run the container"
sleep 10
newPod=$(kubectl get pods | grep "seccon-deployment" | awk '{print $1}')
